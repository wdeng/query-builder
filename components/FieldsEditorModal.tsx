...
export default function FieldsEditorModal({ fields, onChange }: FieldsEditorModalProps) {
  ...
  return (
    <div>
      <button className="rounded border px-2 py-1 text-xs" onClick={() => setOpen(true)}>Edit Fields</button>
      {open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-neutral-900 border rounded shadow-lg w-[720px] max-w-[95vw] p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <button className="rounded border px-2 py-1 text-xs" onClick={() => setOpen(false)}>Close</button>
            </div>
